...
%>
        ...
        rs2=stmt.executeQuery("select * from Guardado where per_id="+request.getParameter("idu")+";");
        ...
        while(rs2.next()){
         ...
         rs = stmt.executeQuery("select n.*,g.* from Feedback g inner join Negocio n where g.per_id="+request.getParameter("idu")+";");
        
                        }else{
                        
                         pid = con.Pid(correo);
                          rs = stmt.executeQuery("select n.*,g.* from Feedback g inner join Negocio n where g.per_id="+pid+";");
                        }
                        ...
                        <section class="w-75 h-100 d-flex flex-column justify-content-between">
                            ...
                            <section class="d-flex justify-content-between">
                                ...
                                <section class="btns_ch">
                                    ...
                                    <form class="coment_pro_ch d-flex" method="post" action="" >
                                    <button type="submit" class="btn btn_elim_busq">Editar comentario</button>
                                    <input type="hidden" name="idc" value="<%=idc%>"/>
                                    </form>
                                    
                                    <form class="coment_pro_ch d-flex" method="post" action="EliminarGuardado" >
                                    <button type="submit" class="btn btn_elim_busq">Eliminar de Guardados</button>
                                    <input type="hidden" name="idn" value="<%=idn%>"/>
                                    </form>
                                    
                                    <form class="coment_pro_ch d-flex" method="post" action="Negocio.jsp" >
